...
for (i in 1:7){
  scgen_pr[[i]] <- pr_dt[[i]][pr_dt[[i]]$method=="scgen",]
  tf_pr[[i]] <- pr_dt[[i]][pr_dt[[i]]$method=="two-factor",]
}

scgen_pr_50 <- c(max(scgen_pr[[1]]$precision[round(scgen_pr[[1]]$recall,2)>=0.5]))
```
...
prediction <- ct_pred[[3]]
dt_r2_ct <- data.frame(genes,observation,prediction,status)
names(dt_r2_ct) <- c("genes", "observation", "prediction", "status")

p_r2_ct <- ggplot(dt_r2_ct, aes(x=prediction, y=observation, color=status)) + 
  geom_point()+
  geom_smooth(method=lm, na.rm = TRUE,colour="black")+
  scale_colour_manual(name="status", values=c("red","blue"))+
  xlab("predicted stimulated expressions") + ylab("In vitro stimulated expressions")+theme_bw()+ theme(legend.position="bottom")+theme(axis.title = element_text(size=12))+ggtitle(TeX(paste("CD4T, $R^2 = $",R2_ct[3])))
  print(p_r2_ct)
```
...
dt_r2$method <- factor(dt_r2$method, labels = c(TeX(paste("scGen, $R^2 = $", R2_scgen[3])),TeX(paste("Two-factor, $R^2 = $", R2_tf[3]))))
  
  p_r2 <- ggplot(dt_r2, aes(x=prediction, y=observation, color=status)) + 
  geom_point()+
  geom_smooth(method=lm, na.rm = TRUE,colour="black")+
  scale_colour_manual(name="status", values=c("red","blue"))+
  xlab("predicted stimulated expressions") + ylab("In vitro stimulated expressions")+facet_wrap(~method,nrow = 1,labeller = label_parsed)+theme_bw()+theme(strip.text = element_text(size=12))+ theme(legend.position="bottom")+theme(axis.title = element_text(size=12))
  print(p_r2)
```
...
cd4t_tb_tf <- cd4t_tb_tf[order(abs(cd4t_tb_tf$logFC),decreasing = TRUE),]
...
vp_iv <- EnhancedVolcano(cd4t_tb_iv,
                         ...
                         selectLab = rownames(cd4t_tb_iv)[1:10],
                         pCutoff=1e-10,
                         FCcutoff = 0.3,
                         colCustom = keyvals,
                         colAlpha = 1,
...
vp_tf <- EnhancedVolcano(cd4t_tb_tf,
                         ...
                         selectLab = cd4t_tb_tf$Row.names[1:10],
                         pCutoff=1e-10,
                         FCcutoff = 0.3,
                         colCustom = keyvals,
                         colAlpha = 1,
...
vp_scgen <- EnhancedVolcano(cd4t_tb_scgen,
                         ...
                         selectLab = rownames(cd4t_tb_scgen)[1:10],
                         pCutoff=1e-10,
                         FCcutoff = 0.3,
                         colCustom = keyvals,
                         colAlpha = 1,
...
r2_lfc_scgen <- round(cor(lfc_dt_cd4t$observation[lfc_dt_cd4t$m=="scGen"],lfc_dt_cd4t$prediction[lfc_dt_cd4t$m=="scGen"])^2,2)
r2_lfc_tf <- round(cor(lfc_dt_cd4t$observation[lfc_dt_cd4t$m=="Two-factor"],lfc_dt_cd4t$prediction[lfc_dt_cd4t$m=="Two-factor"])^2,2)

lfc_dt_cd4t$m <- factor(lfc_dt_cd4t$m, labels = c(TeX(paste("scGen, $R^2 = $", r2_lfc_scgen)),TeX(paste("Two-factor, $R^2 = $", r2_lfc_tf))))
...
p_cfm <- ggplot(lfc_dt_cd4t, aes(x=prediction, y=observation, color=status)) + 
  ...
  geom_smooth(method=lm, na.rm = TRUE,colour="black")+
  theme_bw()+
  theme(strip.text = element_text(size=12))+facet_wrap(.~m, nrow=1,labeller = label_parsed)+theme(axis.title = element_text(size=12))+theme(legend.position = "bottom")
  print(p_cfm)
```
...
library(cowplot)
r1 <- plot_grid(p_r2,p_cfm,nrow=1,ncol=2,labels = c("a","b"), align = "h",axis = "t",label_size = 18)
r2 <- plot_grid(vp_iv, vp_tf, vp_scgen, p_pr, labels = c("c","","","d"), ncol=4,align = "h",label_size = 18)

pdf("CD4T.pdf",height=9,width = 20)
plot_grid(r1,r2,nrow=2, align = "hv",rel_heights = c(1,1.4))
dev.off()
...
```{r}
## R2 barplot for cell type model
cell_type <- c(ctype,ctype)
R2 <- c(R2_ct,R2_DE_ct)
method <- c(rep("All genes",length(ctype)),rep("DE genes",length(ctype)))
dtff <- data.frame(cell_type,R2,method)

pct <- ggplot(dtff, aes(fill=method, y=R2, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = R2),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+
  ylab(TeX("$R^2$: predicted expressions vs in vitro expressions"))+theme_bw()+ theme(legend.position="bottom") + ylim(c(0,1))+guides(fill=guide_legend(title=""))
pct
```
```{r}
pdf("r2_ct.pdf",height = 5, width = 13)
plot_grid(p_r2_ct,NULL,pct,nrow=1,labels = c("a","","b"),label_size = 16,rel_widths = c(1,0.3,1))
dev.off()
```
...
p1 <- ggplot(dtf1, aes(fill=model, y=R2, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = R2),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+
  ylab(TeX("$R^2$: predicted expressions vs in vitro expressions"))+theme_bw()+ theme(legend.position="none") + ylim(c(0,1))
p1
```
...
scgen_lfc_r2 <- c()
for (i in 1:length(ctype)){
  scgen_lfc_r2[i] <- round(cor(dt_scgen_list[[i]]$observation,dt_scgen_list[[i]]$prediction)^2,2)
}

tf_lfc_r2 <- c()
for (i in 1:length(ctype)){
  tf_lfc_r2[i] <- round(cor(dt_tf_list[[i]]$observation,dt_tf_list[[i]]$prediction)^2,2)
}

cell_type <- c(ctype,ctype)
lfc_R2 <- c(scgen_lfc_r2,tf_lfc_r2)
model <- c(rep("scGen",length(ctype)),rep("Two-factor",length(ctype)))
dtf3 <- data.frame(cell_type,lfc_R2,model)

p2 <- ggplot(dtf3, aes(fill=model, y=lfc_R2, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = lfc_R2),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+
  ylab(TeX("$R^2$: predicted logFC vs in vitro logFC"))+ theme(legend.position="none") + ylim(c(0,1))
p2
```
...
p3 <- ggplot(dtf2, aes(fill=model, y=pr_auc, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = pr_auc),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+ylab("AUC-PR") +ylim(c(0,1))+theme_bw()
p3
...
```{r,fig.width=20}
pdf("r2_auc.pdf",height = 5, width = 15)
plot_grid(p1,NULL,p3,nrow=1,rel_widths = c(1,0.3,1.15),labels = c("a","","b"))
dev.off()
```

```{r}
pdf("r2_auc_2.pdf",height = 6, width = 16)
plot_grid(p1,p3,nrow=1,rel_widths = c(1,1.3),labels = c("a","b"))
dev.off()
```

```{r}
## PR curve for all cell types 
