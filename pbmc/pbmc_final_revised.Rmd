...
```{r}
paths <- c("results/pred_NK.csv","results/pred_Dendritic.csv","results/pred_CD4T.csv","results/pred_B.csv","results/pred_FCGR3A.csv","results/pred_CD14.csv","results/pred_CD8T.csv")
scgen_pred <- list()
for (i in 1: length(ctype)){
...
DEG_genes_tf <- list()
for (i in 1: length(ctype)){
  temp <- DEG_tf[[i]]$gene[DEG_tf[[i]]$adj.P.val<p_lvl & abs(DEG_tf[[i]]$logfc)>lfc]
  DEG_genes_tf[[i]] <- temp
}
