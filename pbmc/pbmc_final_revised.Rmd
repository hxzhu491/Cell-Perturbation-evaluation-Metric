...
pdf("CD4T.pdf",height=14,width = 18)
...
p1 <- ggplot(dtf1, aes(fill=model, y=R2, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = R2),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+
  ylab(TeX("$R^2$: predicted expressions vs in vitro expressions"))+ theme(legend.position="none") + ylim(c(0,1))
p1
```
...
p3 <- ggplot(dtf2, aes(fill=model, y=pr_auc, x=cell_type)) + geom_bar(position = "dodge", stat="identity")+geom_text(aes(label = pr_auc),position = position_dodge(width = 0.9), vjust=-0.3)+xlab("Cell type")+ylab("AUC-PR") +ylim(c(0,1))
p3
...
```{r,fig.width=20}
pdf("r2_auc.pdf",height = 6, width = 20)
plot_grid(p1,p2,p3,nrow=1,rel_widths = c(1,1,1.3),labels = c("a","b","c"))
dev.off()
```
