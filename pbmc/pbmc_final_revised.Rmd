...
grid.arrange(vp_iv,vp_tf,vp_scgen, ncol=3)
dev.off()

vp <- grid.arrange(vp_iv,vp_tf,vp_scgen, ncol=3)
...
p <- ggplot(lfc_dt_cd4t, aes(x=prediction, y=observation, color=status)) + 
  geom_point(shape=1,alpha=0.8)+
  geom_hline(yintercept=0.3, linetype="dashed", color = "black")+
  geom_hline(yintercept=-0.3, linetype="dashed", color = "black")+
  geom_vline(xintercept=0.3, linetype="dashed", color = "black")+
  geom_vline(xintercept=-0.3, linetype="dashed", color = "black")+
  xlab("True Logfc") + ylab("Predicted Logfc")+ ggtitle(ctype[[3]])+
  theme(plot.tag.position = "topright")+facet_wrap(~m, nrow=1)
  print(p)
```