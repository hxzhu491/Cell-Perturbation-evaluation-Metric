...
dtf <- data.frame(cell_type,R2,model)

ggplot(dtf, aes(fill=model, y=R2, x=cell_type)) + geom_bar(position = "dodge", stat="identity")
```
```{r}
### auc pr barplot
scgen_pr_auc <- c()
for (i in 1:length(pr_auc_scgen)){
  scgen_pr_auc[i] <- as.numeric(pr_auc_scgen[[i]][1,3])
}

tf_pr_auc <- c()
for (i in 1:length(pr_auc_tf)){
  tf_pr_auc[i] <- as.numeric(pr_auc_tf[[i]][1,3])
}

pr_auc <- c(scgen_pr_auc,tf_pr_auc)
dtf <- data.frame(cell_type,pr_auc,model)

ggplot(dtf, aes(fill=model, y=pr_auc, x=cell_type)) + geom_bar(position = "dodge", stat="identity")
```