...
for (i in 1:100){
  ...
  pred_expr <- pt_pred_full_temp[ix_n,-(1:7)]
  pred_mean <- apply(pred_expr,2,mean)
  r2 <- round(cor(obs_mean,pred_mean)^2,2)
...
```{r}
### cmb for creating R2
cmb_temp <- data.frame(tf_pred_full_temp$cell_type,tf_pred_full_temp$sm_name)
cmb_vec_temp <- paste0(tf_pred_full_temp$cell_type,"#",tf_pred_full_temp$sm_name)
cmb_uni_temp <- unique(cmb_vec_temp)
cmb_list_temp <- strsplit(cmb_uni_temp, split = "#")
...
for (i in 1:100){
  ...
  obs_expr <- bulk_adata_X[ix,]
  obs_mean <- apply(obs_expr,2,mean)
  ix_n <- tf_pred_full_temp$cell_type==cmb_df_temp$cell_type[i] & tf_pred_full_temp$sm_name==cmb_df_temp$sm_name[i]
  pred_expr <- tf_pred_full_temp[ix_n,-(1:7)]
  pred_mean <- apply(pred_expr,2,mean)
  r2 <- round(cor(obs_mean,pred_mean)^2,2)
...
## B cell type
ct_DE_B <- ct_DE[ct_DE$cell_type=="B cells",]
B_Rpert <- as.factor(Rpert[ct_DE$cell_type=="B cells"])
donor_id <- ct_DE_B$donor_id
plate_name <- ct_DE_B$plate_name
row <- ct_DE_B$row

counts <- data.frame(t(ct_DE_B[,-(1:6)]))
counts[counts<0] <- 0
d0 <- DGEList(counts)
d0 <- calcNormFactors(d0)

mm <- model.matrix(~0+B_Rpert+donor_id+plate_name+row)
y <- voom(d0, mm, plot=TRUE)
fit <- lmFit(y, mm)

grp_B <- colnames(coef(fit))[c(1,3:15)]
result_ct_list_B <- list()
for (i in 1:length(grp_B)){
  contr <- makeContrasts(contrasts = paste0(grp_B[i],"-","B_Rpert2"), levels = colnames(coef(fit)))
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_ct_list_B[[i]] <- topTable(tmp,  n=Inf, sort='none')
}

## Myeloid 

ct_DE_Myeloid <- ct_DE[ct_DE$cell_type=="Myeloid cells",]
Myeloid_Rpert <- as.factor(Rpert[ct_DE$cell_type=="Myeloid cells"])
donor_id <- ct_DE_Myeloid$donor_id
plate_name <- ct_DE_Myeloid$plate_name
row <- ct_DE_Myeloid$row

counts <- data.frame(t(ct_DE_Myeloid[,-(1:6)]))
counts[counts<0] <- 0
d0 <- DGEList(counts)
d0 <- calcNormFactors(d0)

mm <- model.matrix(~0+Myeloid_Rpert+donor_id+plate_name+row)
y <- voom(d0, mm, plot=TRUE)
fit <- lmFit(y, mm)

grp_Myeloid <- colnames(coef(fit))[c(1,3:16)]
result_ct_list_Myeloid <- list()
for (i in 1:length(grp_Myeloid)){
  contr <- makeContrasts(contrasts = paste0(grp_Myeloid[i],"-","Myeloid_Rpert2"), levels = colnames(coef(fit)))
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_ct_list_Myeloid[[i]] <- topTable(tmp,  n=Inf, sort='none')
}

## NK

ct_DE_NK <- ct_DE[ct_DE$cell_type=="NK cells",]
NK_Rpert <- as.factor(Rpert[ct_DE$cell_type=="NK cells"])
donor_id <- ct_DE_NK$donor_id
plate_name <- ct_DE_NK$plate_name
row <- ct_DE_NK$row

counts <- data.frame(t(ct_DE_NK[,-(1:6)]))
counts[counts<0] <- 0
d0 <- DGEList(counts)
d0 <- calcNormFactors(d0)

mm <- model.matrix(~0+NK_Rpert+donor_id+plate_name+row)
y <- voom(d0, mm, plot=TRUE)
fit <- lmFit(y, mm)

grp_NK <- colnames(coef(fit))[c(1,3:145)]
result_ct_list_NK <- list()
for (i in 1:length(grp_NK)){
  contr <- makeContrasts(contrasts = paste0(grp_NK[i],"-","NK_Rpert2"), levels = colnames(coef(fit)))
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_ct_list_NK[[i]] <- topTable(tmp,  n=Inf, sort='none')
}
...
```{r,fig.height=10}
## Some volcano plots
for (i in 1: length(result_ct_list_B)){
  df_temp <- data.frame(rownames(result_ct_list_B[[i]]),result_ct_list_B[[i]]$P.Value,result_ct_list_B[[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_B[i],
...
```{r, fig.height=10}
for (i in 1: length(result_list_Myeloid)){
  df_temp <- data.frame(rownames(result_ct_list_Myeloid[[i]]),result_ct_list_Myeloid[[i]]$P.Value,result_ct_list_Myeloid[[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_NK[i],
...
### Get DE result for pert model prediction

```{r}
pt_DE <- data.frame(rbind(control,pt_pred_full_temp[,-1]))

pert <- unique(pt_DE$sm_name)
Rpert_uni <- c(1:length(pert))

Rpert <- c()
for (i in 1:nrow(pt_DE)){
  Rpert[i] <- Rpert_uni[which(pt_DE$sm_name[i]==pert)]
}
```
...
## Use limma for DE analysis
## B cell type
pt_DE_B <- pt_DE[pt_DE$cell_type=="B cells",]
B_Rpert <- as.factor(Rpert[pt_DE$cell_type=="B cells"])
donor_id <- pt_DE_B$donor_id
plate_name <- pt_DE_B$plate_name
row <- pt_DE_B$row

counts <- data.frame(t(pt_DE_B[,-(1:6)]))
counts[counts<0] <- 0
d0 <- DGEList(counts)
d0 <- calcNormFactors(d0)
...
grp_B <- colnames(coef(fit))[c(1,3:10)]
result_pt_list_B <- list()
for (i in 1:length(grp_B)){
  contr <- makeContrasts(contrasts = paste0(grp_B[i],"-","B_Rpert2"), levels = colnames(coef(fit)))
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_pt_list_B[[i]] <- topTable(tmp,  n=Inf, sort='none')
}

## Myeloid 

pt_DE_Myeloid <- pt_DE[pt_DE$cell_type=="Myeloid cells",]
Myeloid_Rpert <- as.factor(Rpert[pt_DE$cell_type=="Myeloid cells"])
donor_id <- pt_DE_Myeloid$donor_id
plate_name <- pt_DE_Myeloid$plate_name
row <- pt_DE_Myeloid$row

counts <- data.frame(t(pt_DE_Myeloid[,-(1:6)]))
counts[counts<0] <- 0
d0 <- DGEList(counts)
d0 <- calcNormFactors(d0)
...
grp_Myeloid <- colnames(coef(fit))[c(1,3:7)]
result_pt_list_Myeloid <- list()
for (i in 1:length(grp_Myeloid)){
  contr <- makeContrasts(contrasts = paste0(grp_Myeloid[i],"-","Myeloid_Rpert2"), levels = colnames(coef(fit)))
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_pt_list_Myeloid[[i]] <- topTable(tmp,  n=Inf, sort='none')
}
...
for (i in 1:length(grp_NK)){
  ...
  tmp <- contrasts.fit(fit, contr)
  tmp <- eBayes(tmp)
  result_list_NK[[i]] <- topTable(tmp,  n=Inf, sort='none')
}
...
```{r,fig.height=10}
## Some volcano plots
for (i in 1: length(result_list_B)){
  df_temp <- data.frame(rownames(result_list_B[[i]]),result_list_B[[i]]$P.Value,result_list_B[[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_B[i],
...
```{r, fig.height=10}
for (i in 1: length(result_pt_list_Myeloid)){
  df_temp <- data.frame(rownames(result_pt_list_Myeloid[[i]]),result_pt_list_Myeloid[[i]]$P.Value,result_pt_list_Myeloid[[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_NK[i],
