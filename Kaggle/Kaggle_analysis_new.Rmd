...
for (i in 1: nrow(d0_test_X)){
  ...
  c.target <- d0_test_full[i,]$cell_type## assume this (a,c) is not observed
  subset_searching <- d0_train_full
...
for (i in 1: nrow(d1_test_X)){
  ...
  c.target <- d1_test_full[i,]$cell_type## assume this (a,c) is not observed
  subset_searching <- d1_train_full
  y_searching <- as.matrix(subset_searching[,-(1:6)])
  a <- sia_find_donor(subset_searching,a.target,c.target)
  b <- sia_find_training(subset_searching,a,a.target,c.target)
...
for (i in 1: nrow(d2_test_X)){
  ...
  c.target <- d2_test_full[i,]$cell_type## assume this (a,c) is not observed
  subset_searching <- d2_train_full
  y_searching <- as.matrix(subset_searching[,-(1:6)])
  a <- sia_find_donor(subset_searching,a.target,c.target)
  b <- sia_find_training(subset_searching,a,a.target,c.target)
...
r2_ct <- c()
for (i in 1:nrow(pairs_test)){
  ix <- bulk_meta$cell_type==pairs_test$cell_type[i] & bulk_meta$sm_name==pairs_test$sm_name[i]
  obs_expr <- bulk_adata_X[ix,]
  obs_mean <- apply(obs_expr,2,mean)
  ix_n <- ct_pred_full$cell_type==pairs_test$cell_type[i] & ct_pred_full$sm_name==pairs_test$sm_name[i]
  pred_expr <- ct_pred_full[ix_n,-(1:6)]
  pred_mean <- apply(pred_expr,2,mean)
  r2_ct[i] <- round(cor(obs_mean,pred_mean)^2,2)
...
for (i in 1:nrow(pairs_test)){
  ...
  obs_expr <- bulk_adata_X[ix,]
  obs_mean <- apply(obs_expr,2,mean)
  ix_n <- pt_pred_full$cell_type==pairs_test$cell_type[i] & pt_pred_full$sm_name==pairs_test$sm_name[i]
  pred_expr <- pt_pred_full[ix_n,-(1:6)]
  pred_mean <- apply(pred_expr,2,mean)
  r2_pt[i] <- round(cor(obs_mean,pred_mean)^2,2)
...
for (i in 1:nrow(pairs_test)){
  ...
  obs_expr <- bulk_adata_X[ix,]
  obs_mean <- apply(obs_expr,2,mean)
  ix_n <- tf_pred_full$cell_type==pairs_test$cell_type[i] & tf_pred_full$sm_name==pairs_test$sm_name[i]
  pred_expr <- tf_pred_full[ix_n,-(1:6)]
  pred_mean <- apply(pred_expr,2,mean)
  r2_tf[i] <- round(cor(obs_mean,pred_mean)^2,2)
...
for (i in 1:nrow(pairs_test)){
  ...
  obs_expr <- bulk_adata_X[ix,]
  obs_mean <- apply(obs_expr,2,mean)
  ix_n <- sia_pred_full$cell_type==pairs_test$cell_type[i] & sia_pred_full$sm_name==pairs_test$sm_name[i]
  pred_expr <- sia_pred_full[ix_n,-(1:6)]
  pred_mean <- apply(pred_expr,2,mean)
  r2_sia[i] <- round(cor(obs_mean,pred_mean)^2,2)
  ...
  geom_smooth(method=lm, na.rm = TRUE,colour="black")+
  xlab("in_vitro_mean") + ylab("in_silico_prediction")+ ggtitle(paste0(pairs_test$cell_type[i],"_",pairs_test$sm_name[i]))+
  labs(tag = paste0("R2=",r2_sia[i]))+ theme(plot.tag.position = "topright")
  print(p)
}
```
...
```{r,fig.height=10}
## Some volcano plots
for (i in 1: length(DE_ct_result[[1]])){
  df_temp <- data.frame(rownames(DE_ct_result[[1]][[i]]),DE_ct_result[[1]][[i]]$P.Value,DE_ct_result[[1]][[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_B[i],
...
```{r, fig.height=10}
for (i in 1: length(DE_sia_result[[1]])){
  df_temp <- data.frame(rownames(DE_sia_result[[1]][[i]]),DE_sia_result[[1]][[i]]$P.Value,DE_sia_result[[1]][[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_B[i],
...
```{r, fig.height=10}
for (i in 1: length(result_ct_list_NK)){
  df_temp <- data.frame(rownames(result_ct_list_NK[[i]]),result_ct_list_NK[[i]]$P.Value,result_ct_list_NK[[i]]$logFC)
  names(df_temp) <- c("gene_name","P.Val","logFC")
  v <- EnhancedVolcano(df_temp,
       lab = gene_name,
       title = grp_NK[i],
...
```{r}
for (i in 1:nrow(pairs_test)){
  ix <- DE_p_value_df$cell_type==pairs_test$cell_type[i] & DE_p_value_df$sm_name==pairs_test$sm_name[i]
  p_iv <- DE_p_value_df[ix,]
  lfc_iv <- DE_lfc[ix,]
}
```
