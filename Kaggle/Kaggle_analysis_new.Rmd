...
ggplot(dt, aes(x=method, y= r2)) + geom_dotplot(binaxis = "y", stackdir = "center", stackratio = 0.6, dotsize = 0.3, fill="#FFAAD4")+stat_summary(fun.data="mean_sdl", fun.args = list(mult=1), geom="pointrange",color="blue")+stat_summary(fun.y=mean, geom="label", aes(label = round(..y.., 2)), hjust = -1, vjust=-0.1)+scale_x_discrete(limits=c("cell type","perturbation","two-factor","SIA"))+facet_wrap(~d)+theme_bw()
dev.off()
```
...
##dot plot PR_AUC
mthd <- c(rep("cell type",100),rep("perturbation",100),rep("two-factor",100),rep("SIA",100))
prauc <- c(pr_auc_ct,pr_auc_pt,pr_auc_tf,pr_auc_sia)
prauc <- round(prauc,2)
dt <- data.frame(mthd,prauc)
names(dt) <- c("method","AUC-PR")

pdf("dot_auc.pdf",height = 5, width = 7)
ggplot(dt, aes(x=method, y= prauc)) + geom_dotplot(binaxis = "y", stackdir = "center", stackratio = 0.6, dotsize = 0.3, fill="#FFAAD4")+stat_summary(fun.data="mean_sdl", fun.args = list(mult=1), geom="pointrange",color="blue")+stat_summary(fun.y=mean, geom="label", aes(label = round(..y.., 2)), hjust = -1, vjust=-0.1)+scale_x_discrete(limits=c("cell type","perturbation","two-factor","SIA"))+theme_bw()
dev.off()
...
dt_r2$method <- factor(dt_r2$method, labels = c(TeX(paste("Cell type, $R^2 = $", r2_ct_d0[42])),TeX(paste("Perturbation, $R^2 = $", r2_pt_d0[42])),TeX(paste("Two-factor, $R^2 = $", r2_tf_d0[42])),TeX(paste("SIA, $R^2 = $", r2_sia_d0[42]))))
  
  p_r2 <- ggplot(dt_r2, aes(x=prediction, y=observation, color=status)) + 
  geom_point()+
  geom_smooth(method=lm, na.rm = TRUE,colour="black")+
  scale_colour_manual(name="status", values=c("red","blue"))+
  xlab("predicted stimulated expressions") + ylab("In vitro stimulated expressions")+facet_wrap(~method,nrow = 2,labeller = label_parsed)+theme_bw()+theme(strip.text = element_text(size=12))+ theme(legend.position="bottom")+theme(axis.title = element_text(size=12))
  print(p_r2)
```
